...
namespace i18n.Sample
{
    ...
    public static class AlertsBarExtensions
    {
        ...
        private static BarData Error =
            new BarData() { Kind = "error", CssClassName = "alert-error", MessageDelegate = () => "Error de validación" };
            ...
            bool includePropertyErrors = true, object htmlAttributes = null)
        {
            ...
            foreach (var sb in Alerts)
            {
				...
				if ( (@this.ViewContext.TempData[GetTempDataKeyFor(sb)] as IList<string>) == null || !(@this.ViewContext.TempData[GetTempDataKeyFor(sb)] as IList<string>).Any())
				{
					continue;
				}
